...
def load_investor_analysis(investor):
    st.title(investor)
    df_5=df[df['investors'].str.contains(investor)][['date','startup','vertical','city','amount']].head()
    st.dataframe(df_5)
...
if options=="Overall":
    ...
    selected_investor=st.sidebar.selectbox('Investor Name',sorted(set(df['investors'].str.split(",").sum())))
